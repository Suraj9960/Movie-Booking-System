...
<body>
	...
	<h1 class="text-center text-[20px] font-bold py-4">Movie Name :
		${movie.movieName}</h1>
	<a href="/user/book-ticket?id=${movie.id}"
	...
	<div class="container mt-4 mx-auto mb-4">
		<div class="container ">
			<c:if test="${noShowsFound}">
				...
				<p
					class="text-center  text-red-700 text-[20px] font-bold">!!
					NO SHOW FOUND FOR MOVIE !!</p>
			...
			<c:forEach items="${allShows}" var="show">
				<form action="bookShow" method="post">
					<input type="hidden" name="id" value="${show.movie.id}"> <input
						type="hidden" name="theaterId" value="${show.theater.tid}">
					<input type="hidden" name="tseats" value="${show.theater.tseats}">
					...
					<div class="mt-4 mb-4">
							...
							<div class="md:flex">
								<div class="md:shrink-0">
									<img class="h-48 w-full object-cover md:h-full md:w-48"
										src="../image/${image}" alt="Modern building architecture">
								</div>
								<div class="p-8">
									<div
										class="uppercase tracking-wide text-sm text-indigo-500 font-bold block mt-1 text-lg leading-tight font-medium">${show.movie.movieName}</div>
									<p
										class="block mt-1 text-md leading-tight font-medium text-black ">${show.theater.tname}
										&nbsp;&nbsp;${show.dateOfShow}</p>
									<p class="mt-2 text-slate-500">
									<p class="mb-2">${show.showName}&nbsp;(${show.showStartTime}
										-- ${show.showEndTime} )</p>
									<button type="submit"
										class="text-center p-2 bg-green-700 text-white rounded-lg">
										Book Show&nbsp;<i class="fa-solid fa-circle-chevron-right"></i>
									</button>
						...
						<!-- Hidden input field to capture the selected show name -->
						<input type="hidden" name="selectedShow" value="${show.showName}">
						<input type="hidden" name="showId" value="${show.sid}"> <input
							type="hidden" name="showdate" value="${show.dateOfShow}">
